...
class Point {
  ...
  */
  ...
  // Returns a {Point}.
  translate(other) {
    let {row, column} = Point.fromObject(other);
  ...
  traverse(other) {
    ...
    let row = this.row + other.row;
    if (other.row === 0) {
      var column = this.column + other.column;
    } else {
      var { column } = other;
    }
  ...
  splitAt(column) {
    if (this.row === 0) {
      var rightColumn = this.column - column;
    } else {
      var rightColumn = this.column;
    }
