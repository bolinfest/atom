...
class Range {
  ...
  */
  ...
  static fromText(...args) {
    if (args.length > 1) {
      var startPoint = Point.fromObject(args.shift());
    } else {
      var startPoint = new Point(0, 0);
    }
    let text = args.shift();
    let endPoint = startPoint.copy();
    let lines = text.split(newlineRegex);
    if (lines.length > 1) {
  ...
  */
  ...
  union(otherRange) {
    let start = this.start.isLessThan(otherRange.start) ? this.start : otherRange.start;
    let end = this.end.isGreaterThan(otherRange.end) ? this.end : otherRange.end;
  ...
  */
  ...
  containsRange(otherRange, exclusive) {
    let {start, end} = this.constructor.fromObject(otherRange);
  ...
  */
  ...
  toDelta() {
    let rows = this.end.row - this.start.row;
    if (rows === 0) {
      var columns = this.end.column - this.start.column;
    } else {
      var columns = this.end.column;
    }
